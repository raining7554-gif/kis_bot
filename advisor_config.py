...
# ─────────────────────────────────────────────────────────
# 백테스트(2019~) 반영: 추세추종형 — 넓은 손절 12%, 매수후보 최소 65점.
# 청산은 고정목표가 아니라 '일봉 MA20 이탈'(analyze_swing이 산출).
SW_STOP_PCT   = float(os.environ.get("SW_STOP_PCT", "0.12"))   # 하드 손절(재난방어)
SW_RR         = float(os.environ.get("SW_RR", "1.5"))          # 참고 목표 계산용
SW_MIN_SCORE  = int(os.environ.get("SW_MIN_SCORE", "65"))      # 이 점수 미만은 '관망'
